...
def levelUp(character):
        ...
        print('current attributes: strength ' + str(character.strength) + ', dexterity ' + str(character.dexterity) + ', constitution ' + str(character.constitution)
        + ', intelligence ' + str(character.intelligence) + ', wisdom ' + str(character.wisdom) + ', charisma ' + str(character.charisma))
        ...
        character.strength, character.dexterity, character.constitution, character.intelligence, character.wisdom, character.charisma = (x for x in character.attributes)
        ...
        (character.acting, character.anthropology, character.xenoanthropology, character.diplomacy,
        character.medicine, character.vehicles, character.technology, character.xenotechnology, character.sleightofhand, character.stealth,
        character.insight, character.perception, character.survival, character.tactics, character.athletics, character.acrobatics) = (x for x in character.skills)
...
def load():
    ...
    for faction in os.listdir("./factions"):
        ...
        for character in os.listdir('./factions/'+faction):
            ...
            with open('./factions/'+faction+'/'+character, 'r', encoding='utf-8', errors='ignore') as file:
...
def savecharacters(characters):
    ...
    for character in characters:
        ...
        Path('.\\factions\\'+character.faction+'\\'+character.name+'.json').touch()

        # write
        f = open('.\\factions\\'+character.faction+'\\'+character.name+'.json', 'w', encoding='utf-8', errors='ignore')
...
def attack(attacker, defender, weapon, distance=0, cover=None):
    ...
    if hitcalc > (defender.AC + covermod):
        # damage calculation
        damage = round(
            (randint(1, weapon.damage // 2) + randint(1, weapon.damage // 2))
            ) * distancemod
        
        print(attacker.name + ' rolled ' + str(hitcalc) + ' to hit ' + defender.name + ' for ' + str(damage) + '!')
